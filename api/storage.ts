...
export class MemStorage {
  ...
  initializeData() {
    ...
    sampleProxies.forEach(p => this.proxyLinks.set(p.id, p));
  ...
  async getProxyLinks() {
    return Array.from(this.proxyLinks.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }

  async getActiveProxyLinks() {
  ...
  async deleteProxyLink(id: string) {
    return this.proxyLinks.delete(id);
  }

  async getAnnouncements() {
    return Array.from(this.announcements.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }
  ...
  async createAnnouncement(insertAnnouncement: any) {
    const id = randomUUID();
    const announcement = { ...insertAnnouncement, id, createdAt: new Date() };
  ...
  async createFeedback(insertFeedback: any) {
    const id = randomUUID();
    const feedback = {
      ...insertFeedback,
      id,
